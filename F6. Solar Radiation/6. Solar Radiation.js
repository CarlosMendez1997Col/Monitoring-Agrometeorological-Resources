// Monitoring-Agrometeorological-Resources (P-RAIN, Evap-Etp, SR, RH, SM, ST, SWS, SAT, VP, WS)
...
var scaleImage_SR1 = function(image) {return image.divide(2721600).copyProperties(image,['system:time_start']);};
...
var SR = ee.ImageCollection('ECMWF/ERA5_LAND/DAILY_AGGR')
                            .filterDate(startDate, endDate)
                            .select(['surface_net_solar_radiation_sum'])
                            .map(scaleImage_SR1);

var byMonth_SR = ee.ImageCollection(ee.List.sequence(0, nMonths).map(function (n){
                var ini = startDate.advance(n,'month');
                var end = ini.advance(1,'month');
                return SR.filterDate(ini,end)
                              .select(0).mean().rename('Monthly SR')
                              .set('system:time_start', ini)
                              .clip(Region);
                }));

// Create a time-series with a trendline
var monthly_SR = ui.Chart.image.series({
  imageCollection: byMonth_SR,
  region: geometry,
  reducer: ee.Reducer.mean(),
  scale: 5566,
   }).setOptions({
    title: 'Monthly Average Solar Radiation in Colombia',
                                 hAxis: {
                                      title: '',
                                      format: 'Y',
                                ...
                                chartArea: {backgroundColor: ''},
                                  trendlines: {0: 
                                    ...
                                    labelInLegend: 'Monthly SR Trend'}
                                        },
                })
      .setChartType('ColumnChart');
print(monthly_SR);

var scaleImage_SR2 = function(image) {return image.divide(2635200).copyProperties(image,['system:time_start']);};

////////////////////////////////////////////////////////////////////// Annual SR Average//////////////////////////////////////////////////////////////////

var Annual_SR = ee.ImageCollection('ECMWF/ERA5_LAND/DAILY_AGGR')
                 .select(['surface_net_solar_radiation_sum'])
                 .map(scaleImage_SR2);
  ...
  var seasonFiltered = Annual_SR.filter(ee.Filter.date(startDate, endDate));
  var total = seasonFiltered.reduce(ee.Reducer.mean()).rename('Annual SR');
    return total.set({
...
var yearlyCol_SR = ee.ImageCollection.fromImages(yearlyImages);


// Create a time-series with a trendline
var annual_SR = ui.Chart.image.series({
  imageCollection: yearlyCol_SR,
  region: geometry,
  reducer: ee.Reducer.mean(),
  scale: 5566,
   }).setOptions({
    title: 'Annual Average Solar Radiation in Colombia',
                                 hAxis: {
                                      title: '',
                                      format: 'Y',
                                      titleTextStyle: {italic: false, bold: true},
...
var visParams = { min: 0, max: 7, palette: ['blue', 'cyan', 'green', 'yellow', 'orange', 'red']};

var annotations = [{position: 'right', offset: '5%', margin: '20%', property: 'label', scale: 5000}]


// 2. Load an ImageCollection
var byMonth_SR_Tim = byMonth_SR.filter(ee.Filter.date('2015-01-01T00:00:00', '2024-12-31T00:00:00'));

var collection_SR = byMonth_SR_Tim.map(function(img){
  var date = img.get('system:time_start');
  var date2 = ee.Date(date);
  return img.set('system_time_start', date2);
});

var series_SR =  collection_SR.map(function(image){
...
print(rgbVis_SR.limit(180).getVideoThumbURL(videoParams));
print(ui.Thumbnail(rgbVis_SR.limit(180), videoParams));
...
var firstImage = byMonth_SR_Tim.first();

// Get the latest image of collection
var sortedCollection = byMonth_SR_Tim.sort('system:time_start', false);
...
snazzy.addStyle(MidNight,"MidNight", map2);
