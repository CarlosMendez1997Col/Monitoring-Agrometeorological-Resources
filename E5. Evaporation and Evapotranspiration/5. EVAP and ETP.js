...
// Evaporation (EVAP) and Evapotranspiration (Etp)
...
var scaleImage_Evap = function(image) {return image.multiply(-1000).copyProperties(image,['system:time_start']);};
...
var byMonth_Evap = ee.ImageCollection(ee.List.sequence(0, nMonths).map(function (n){
                ...
                return Evap.filterDate(ini,end)
                              .select(0).sum().rename('Monthly Evap')
...
var monthly_evap = ui.Chart.image.series({
  ...
  scale: 5566,
    ...
    title: 'Monthly Average Evaporation in Colombia',
                                 hAxis: {
                                      title: '',
                                      format: 'Y',
                                ...
                                chartArea: {backgroundColor: ''},
                                  trendlines: {0: 
                                    ...
                                    labelInLegend: 'Monthly Evap Trend'}
                                        },
                })
...
var Annual_Evap = ee.ImageCollection('ECMWF/ERA5_LAND/DAILY_AGGR')
  ...
  var total = seasonFiltered.reduce(ee.Reducer.sum()).rename('Annual Evap');
    return total.set({
...
var annual_evap = ui.Chart.image.series({
  ...
  scale: 5566,
    ...
    title: 'Annual Average of Evaporation in Colombia',
                                 hAxis: {
                                      title: '',
                                      format: 'Y',
                                      titleTextStyle: {italic: false, bold: true},
                                ...
                                chartArea: {backgroundColor: ''},
                                  trendlines: {0: 
                                    ...
                                    pointSize: 0,
                                    visibleInLegend: true,
                                    labelInLegend: 'Annual Evap Trend'}
                                        },
                })
      .setChartType('ColumnChart');
print(annual_evap);
...
var visParams = { min: 0, max: 140, palette: ['FFFFFF', 'CE7E45', 'DF923D', 'F1B555', 'FCD163', '99B718', '74A901',
...
var series_Evap =  collection_Evap.map(function(image){
  ...
  return image.visualize({
      ...
      bands: ['Monthly Evap'],
      min: 0, max: 140.0, palette: visParams.palette
  }).set({label: label}); 
});
...
// Gradient Bar
var labels = ee.List.sequence(0, 140, 70);
