...
var WS = ee.ImageCollection('NASA/FLDAS/NOAH01/C/GL/M/V001')
                            .filterDate(startDate, endDate)
                            .select(['Wind_f_tavg'])
//                            .map(scaleImage_WS);

var byMonth_WS = ee.ImageCollection(ee.List.sequence(0, nMonths).map(function (n){
                var ini = startDate.advance(n,'month');
                var end = ini.advance(1,'month');
                return WS.filterDate(ini,end)
                              .select(0).mean().rename('Monthly WS')
                              .set('system:time_start', ini)
                              .clip(Region);
                }));

// Create a time-series with a trendline
var monthly_WS = ui.Chart.image.series({
  imageCollection: byMonth_WS,
  region: geometry,
  reducer: ee.Reducer.mean(),
  scale: 5566,
   }).setOptions({
    title: 'Monthly Average Wind Speed in Colombia',
                                 hAxis: {
                                      title: '',
                                      format: 'Y',
                                ...
                                chartArea: {backgroundColor: ''},
                                  trendlines: {0: 
                                    ...
                                    labelInLegend: 'Monthly WS Trend'}
                                        },
                })
...
////////////////////////////////////////////////////////////////////// Annual Wind Speed Average//////////////////////////////////////////////////////////////////

var Annual_WS = ee.ImageCollection('NASA/FLDAS/NOAH01/C/GL/M/V001')
                 .select(['Wind_f_tavg'])
//                 .map(scaleImage_WS);
  ...
  var seasonFiltered = Annual_WS.filter(ee.Filter.date(startDate, endDate));
  var total = seasonFiltered.reduce(ee.Reducer.mean()).rename('Annual WS');
    return total.set({
...
var yearlyCol_WS = ee.ImageCollection.fromImages(yearlyImages);


// Create a time-series with a trendline
var annual_WS = ui.Chart.image.series({
  imageCollection: yearlyCol_WS,
  region: geometry,
  reducer: ee.Reducer.mean(),
  scale: 5566,
   }).setOptions({
    title: 'Annual Average Wind Speed Colombia',
                                 hAxis: {
                                      title: '',
                                      format: 'Y',
                                      titleTextStyle: {italic: false, bold: true},
...
var visParams = { min: 0, max: 4, palette: ['#c1d8d9', '#c2d5da', '#bcd9dc', '#c1cdcd', '#c5d0e6', '#b7c9e2', '#c1dbea', '#c0e8d5', '#c7cce7']};

var annotations = [{position: 'right', offset: '5%', margin: '20%', property: 'label', scale: 5000}]


// 2. Load an ImageCollection
var byMonth_WS_Tim = byMonth_WS.filter(ee.Filter.date('2015-01-01T00:00:00', '2024-12-31T00:00:00'));

var collection_WS = byMonth_WS_Tim.map(function(img){
  var date = img.get('system:time_start');
  var date2 = ee.Date(date);
  return img.set('system_time_start', date2);
});

var series_WS =  collection_WS.map(function(image){
...
print(rgbVis_WS.limit(180).getVideoThumbURL(videoParams));
print(ui.Thumbnail(rgbVis_WS.limit(180), videoParams));
...
var firstImage = byMonth_WS_Tim.first();

// Get the latest image of collection
var sortedCollection = byMonth_WS_Tim.sort('system:time_start', false);
