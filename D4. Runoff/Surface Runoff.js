// Monitoring-Agrometeorological-Resources (P-RAIN, Evap-ETP, RH, SM, ST, SWS, SAT, Run, WS)

// Created by Carlos Mendez

// Runoff Storm Surface 
...
var scaleImage_Run_S = function(image) {return image.multiply(1000).copyProperties(image,['system:time_start']);};

////////////////////////////////////////////////////////////////////// Monthly Runoff Surface Average//////////////////////////////////////////////////////////////////
...
var Run_S = ee.ImageCollection('ECMWF/ERA5_LAND/DAILY_AGGR')
                            .filterDate(startDate, endDate)
                            .select(['surface_runoff_sum'])
                            .map(scaleImage_Run_S)

var byMonth_Run_S = ee.ImageCollection(ee.List.sequence(0, nMonths).map(function (n){
                var ini = startDate.advance(n,'month');
                var end = ini.advance(1,'month');
                return Run_S.filterDate(ini,end)
                              .select(0).sum().rename('Monthly Run_S')
                              .set('system:time_start', ini)
                              .clip(Region);
                }));

// Create a time-series with a trendline
var monthly_Run_S = ui.Chart.image.series({
  imageCollection: byMonth_Run_S,
  region: geometry,
  reducer: ee.Reducer.mean(),
  scale: 5566,
   }).setOptions({
    title: 'Monthly Average Surface Runoff Colombia',
                                 hAxis: {
                                      title: '',
                                      format: 'Y',
                                ...
                                chartArea: {backgroundColor: ''},
                                  trendlines: {0: 
                                    ...
                                    labelInLegend: 'Monthly Run_S Trend'}
                                        },
                })
...
////////////////////////////////////////////////////////////////////// Annual Runoff Surface Average//////////////////////////////////////////////////////////////////

var Annual_Run_S = ee.ImageCollection('ECMWF/ERA5_LAND/DAILY_AGGR')
                 .select(['surface_runoff_sum'])
                 .map(scaleImage_Run_S)
  ...
  var seasonFiltered = Annual_Run_S.filter(ee.Filter.date(startDate, endDate));
  var total = seasonFiltered.reduce(ee.Reducer.sum()).rename('Annual Run_S');
    return total.set({
...
var yearlyCol_Run_S = ee.ImageCollection.fromImages(yearlyImages);


// Create a time-series with a trendline
var annual_Run_S = ui.Chart.image.series({
  imageCollection: yearlyCol_Run_S,
  region: geometry,
  reducer: ee.Reducer.mean(),
  scale: 5566,
   }).setOptions({
    title: 'Annual Average Surface Runoff in Colombia',
                                 hAxis: {
                                      title: '',
                                      format: 'Y',
                                      titleTextStyle: {italic: false, bold: true},
                                ...
                                chartArea: {backgroundColor: ''},
                                  trendlines: {0: 
                                    ...
                                    pointSize: 0,
                                    visibleInLegend: true,
                                    labelInLegend: 'Annual Run_S Trend'}
                                        },
                })
      .setChartType('ColumnChart');
print(annual_Run_S);
...
var visParams = { min: 0, max: 450, palette: ['af0000','eb1e00','ff6400','ffb300','ffeb00',
                                              '9beb4a','33db80','00b4ff','0064ff','000096',]};

var annotations = [{position: 'right', offset: '5%', margin: '20%', property: 'label', scale: 5000}]


// 2. Load an ImageCollection
var byMonth_Run_S_Tim = byMonth_Run_S.filter(ee.Filter.date('2015-01-01T00:00:00', '2024-12-31T00:00:00'));

var collection_Run_S = byMonth_Run_S_Tim.map(function(img){
  var date = img.get('system:time_start');
  var date2 = ee.Date(date);
  return img.set('system_time_start', date2);
});

var series_Run_S =  collection_Run_S.map(function(image){
  ...
  return image.visualize({
      ...
      bands: ['Monthly Run_S'],
      min: 0, max: 100, palette: visParams.palette
  }).set({label: label}); 
});


series_Run_S = series_Run_S.map(function(image) {
  return text.annotateImage(image, {}, dateLocation, annotations) 
});

var rgbVis_Run_S = series_Run_S.map(function(img)
{
var scale = Map.getScale();

// Title
var label_title = 'Surface Runoff';
var title_text = text.draw(label_title, titleLocation, scale*0.8, {fontSize: 12});

// Text label
var textGradient = 'Milimeters';
var text_Gradient = text.draw(textGradient, gradientLabelLocation, scale*0.8, {fontSize: 10});

// Gradient Bar
var labels = ee.List.sequence(0, 100, 100);
...
print(rgbVis_Run_S.limit(180).getVideoThumbURL(videoParams));
print(ui.Thumbnail(rgbVis_Run_S.limit(180), videoParams));
...
var firstImage = byMonth_Run_S_Tim.first();

// Get the latest image of collection
var sortedCollection = byMonth_Run_S_Tim.sort('system:time_start', false);
...
snazzy.addStyle(AImap,"AImap", map1);
