...
var scaleImage_PRCP = function(image) {return image.multiply(1).copyProperties(image,['system:time_start']);};
...
var byMonth_Prcp = ee.ImageCollection(ee.List.sequence(0, nMonths).map(function (n){
                var ini = startDate.advance(n,'month');
                var end = ini.advance(1,'month');
                return Prcp.filterDate(ini,end)
                              .select(0).sum().rename('Monthly Prcp')
                              .set('system:time_start', ini)
                              .clip(Region);
                }));

// Create a time-series with a trendline
var monthly_Prcp = ui.Chart.image.series({
  imageCollection: byMonth_Prcp,
  region: geometry,
  reducer: ee.Reducer.mean(),
  scale: 5566,
   }).setOptions({
    title: 'Monthly Average Precipitation in Colombia',
                                 hAxis: {
                                      title: '',
                                      format: 'Y',
                                ...
                                chartArea: {backgroundColor: ''},
                                  trendlines: {0: 
                                    ...
                                    labelInLegend: 'Monthly Prcp Trend'}
                                        },
                })
...
var Annual_Prcp = ee.ImageCollection('UCSB-CHG/CHIRPS/DAILY')
                 .select(['precipitation'])
                 .map(scaleImage_PRCP);
  ...
  var seasonFiltered = Annual_Prcp.filter(ee.Filter.date(startDate, endDate));
  var total = seasonFiltered.reduce(ee.Reducer.sum()).rename('Annual Prcp');
    return total.set({
...
var yearlyCol_Prcp = ee.ImageCollection.fromImages(yearlyImages);


// Create a time-series with a trendline
var annual_Prcp = ui.Chart.image.series({
  imageCollection: yearlyCol_Prcp,
  region: geometry,
  reducer: ee.Reducer.mean(),
  scale: 5566,
   }).setOptions({
    title: 'Annual Average of Precipitation in Colombia',
                                 hAxis: {
                                      title: '',
                                      format: 'Y',
                                      titleTextStyle: {italic: false, bold: true},
                                ...
                                chartArea: {backgroundColor: ''},
                                  trendlines: {0: 
                                    ...
                                    pointSize: 0,
                                    visibleInLegend: true,
                                    labelInLegend: 'Annual Prcp Trend'}
                                        },
                })
      .setChartType('ColumnChart');
print(annual_Prcp);
...
var visParams = { min: 0, max: 450, palette: ['000096','0064ff', '00b4ff', '33db80', '9beb4a',
...
var byMonth_Prcp_Tim = byMonth_Prcp.filter(ee.Filter.date('2015-01-01T00:00:00', '2024-12-31T00:00:00'));

var collection_Prcp = byMonth_Prcp_Tim.map(function(img){
  var date = img.get('system:time_start');
  var date2 = ee.Date(date);
  return img.set('system_time_start', date2);
});

var series_Prcp =  collection_Prcp.map(function(image){
  ...
  return image.visualize({
      ...
      bands: ['Monthly Prcp'],
      min: 0, max: 450, palette: visParams.palette
  }).set({label: label}); 
});


series_Prcp = series_Prcp.map(function(image) {
  return text.annotateImage(image, {}, dateLocation, annotations) 
});

var rgbVis_Prcp = series_Prcp.map(function(img)
{
var scale = Map.getScale();

// Title
var label_title = 'Precipitation';
...
// Gradient Bar
var labels = ee.List.sequence(0, 450, 450);
...
print(rgbVis_Prcp.limit(180).getVideoThumbURL(videoParams));
print(ui.Thumbnail(rgbVis_Prcp.limit(180), videoParams));
...
var firstImage = byMonth_Prcp_Tim.first();

// Get the latest image of collection
var sortedCollection = byMonth_Prcp_Tim.sort('system:time_start', false);
...
snazzy.addStyle(Outrun,"Outrun", map1);
...
snazzy.addStyle(GeoMap,"GeoMap", map2);
