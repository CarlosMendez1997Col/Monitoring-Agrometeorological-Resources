...
var scaleImage_SAT = function(image) {return image.subtract(273.15).copyProperties(image,['system:time_start']);};
...
var SAT = ee.ImageCollection('ECMWF/ERA5_LAND/DAILY_AGGR')
                            .filterDate(startDate, endDate)
                            .select(['temperature_2m'])
                            .map(scaleImage_SAT)

var byMonth_SAT = ee.ImageCollection(ee.List.sequence(0, nMonths).map(function (n){
                var ini = startDate.advance(n,'month');
                var end = ini.advance(1,'month');
                return SAT.filterDate(ini,end)
                              .select(0).mean().rename('Monthly SAT')
                              .set('system:time_start', ini)
                              .clip(Region);
                }));

// Create a time-series with a trendline
var monthly_SAT = ui.Chart.image.series({
  imageCollection: byMonth_SAT,
  region: geometry,
  reducer: ee.Reducer.mean(),
  scale: 5566,
   }).setOptions({
    title: 'Monthly Average SAT in Colombia',
                                 hAxis: {
                                      title: '',
                                      format: 'Y',
                                ...
                                chartArea: {backgroundColor: ''},
                                  trendlines: {0: 
                                    ...
                                    labelInLegend: 'Monthly SAT Trend'}
                                        },
                })
...
var Annual_SAT = ee.ImageCollection('ECMWF/ERA5_LAND/DAILY_AGGR')
                  .select(['temperature_2m'])
                  .map(scaleImage_SAT)
  ...
  var seasonFiltered = Annual_SAT.filter(ee.Filter.date(startDate, endDate));
  var total = seasonFiltered.reduce(ee.Reducer.mean()).rename('Annual SAT');
    return total.set({
...
var yearlyCol_SAT = ee.ImageCollection.fromImages(yearlyImages);


// Create a time-series with a trendline
var annual_SAT = ui.Chart.image.series({
  imageCollection: yearlyCol_SAT,
  region: geometry,
  reducer: ee.Reducer.mean(),
  scale: 5566,
   }).setOptions({
    title: 'Annual Average of SAT in Colombia',
                                 hAxis: {
                                      title: '',
                                      format: 'Y',
                                      titleTextStyle: {italic: false, bold: true},
                                ...
                                chartArea: {backgroundColor: ''},
                                  trendlines: {0: 
                                    ...
                                    pointSize: 0,
                                    visibleInLegend: true,
                                    labelInLegend: 'Annual SAT Trend'}
                                        },
                })
      .setChartType('ColumnChart');
print(annual_SAT);
...
var visParams = { min: 0, max: 30, palette: ['#ADD8E6', '#008000', '#FFFF00', '#FFA500', '#FF0000', '#800080']};

var annotations = [{position: 'right', offset: '5%', margin: '20%', property: 'label', scale: 5000}]


// 2. Load an ImageCollection
var byMonth_SAT_Tim = byMonth_SAT.filter(ee.Filter.date('2015-01-01T00:00:00', '2024-12-31T00:00:00'));

var collection_SAT = byMonth_SAT_Tim.map(function(img){
  var date = img.get('system:time_start');
  var date2 = ee.Date(date);
  return img.set('system_time_start', date2);
});

var series_SAT =  collection_SAT.map(function(image){
  ...
  return image.visualize({
      ...
      bands: ['Monthly SAT'],
      min: 0, max: 30, palette: visParams.palette
  }).set({label: label}); 
});


series_SAT = series_SAT.map(function(image) {
  return text.annotateImage(image, {}, dateLocation, annotations) 
});

var rgbVis_SAT = series_SAT.map(function(img)
{
var scale = Map.getScale();

// Title
var label_title = 'Maximum Temperature';
var title_text = text.draw(label_title, titleLocation, scale*0.8, {fontSize: 12});

// Text label
var textGradient = 'Degree Celsius';
var text_Gradient = text.draw(textGradient, gradientLabelLocation, scale*0.8, {fontSize: 10});

// Gradient Bar
var labels = ee.List.sequence(0, 30, 30);
...
print(rgbVis_SAT.limit(180).getVideoThumbURL(videoParams));
print(ui.Thumbnail(rgbVis_SAT.limit(180), videoParams));
...
var firstImage = byMonth_SAT_Tim.first();

// Get the latest image of collection
var sortedCollection = byMonth_SAT_Tim.sort('system:time_start', false);
...
snazzy.addStyle(AccessCall,"AccessCall", map1);
